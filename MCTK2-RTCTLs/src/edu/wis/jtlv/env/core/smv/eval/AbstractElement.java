...
public abstract class AbstractElement {
	...
	protected void pushNextDown(SMVModule a_context, AbstractElement grand,
			AbstractElement par) throws SMVParseException {
		if (par instanceof OpNext) {
			...
			AbstractElement duplicateThis=this.duplicate();//eval Define Statement,It must have a copy version.
			grand.nSetChild(par_idx, duplicateThis);

			for (int i = 0; i < duplicateThis.nGetChildren().length; i++) {
				try {
					AbstractElement child = duplicateThis.nGetChildren()[i].duplicate();
					AbstractElement new_child = new OpNext(child);
					duplicateThis.nSetChild(i, new_child);
					child.pushNextDown(a_context, duplicateThis, new_child);
