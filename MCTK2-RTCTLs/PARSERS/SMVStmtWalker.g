...
//			| ^(BIT_SELECT_T f=number s=number { $ret += $f.text + ":" + $s.text; } )
//			;
...
// FIXME: curr_var_name is not a String anymore (the procedures expect String)
type returns[SMVAbstractElementInfo ret]
			...
			| ^(TOK_WORD se=simple_expression)
				{ $ret = intr.mk_word_var(curr_var_category, curr_var_decl_is_visible, curr_var_name, "2", $se.text, input); }
			| sr=subrange
				{ $ret = intr.mk_range_var(curr_var_category, curr_var_decl_is_visible, curr_var_name, $sr.ret.elementAt(0), $sr.ret.elementAt(1), input); }
			| ^(TYPE_VALUE_LIST_T tvl=type_value_list)
				{ $ret = intr.mk_values_var(curr_var_category, curr_var_decl_is_visible, curr_var_name, $tvl.ret, input); }
			//modules
			...
			| ^(SYNC_PROCESS_T
				...
				// FIXME: $simple_list_expression return something else
			...
			| ^(TOK_ARRAY sr=subrange ri=type)
				{ $ret = intr.mk_range_array_var(curr_var_category, curr_var_decl_is_visible, curr_var_name, $ri.ret, $sr.ret.elementAt(0), $sr.ret.elementAt(1), input); }
			;
...
type_value_list returns[Vector<String> ret]
			: { ret = new Vector<String>(10); }
				( type_value { ret.add($type_value.text); } )+
			;
...
declarations
			: (declaration)*;
declaration	: 	{ curr_var_decl_is_visible = true; curr_var_category = STATE_VAR; } var
			| isa //not implemented in JTLV
			| { curr_var_decl_is_visible = true; curr_var_category = INPUT_VAR; } input_var
			| assign 
			| init
			| invar //not implemented in JTLV
			| trans
			| { curr_var_decl_is_visible = true; curr_var_category = NULL; } define
...
agent_declaration		: 
		 { curr_var_decl_is_visible = true; curr_var_category = STATE_VAR; } var 
		| { curr_var_decl_is_visible = false; curr_var_category = STATE_VAR; } agent_invisible_var
		| { curr_var_decl_is_visible = true; curr_var_category = INPUT_VAR; } input_var
		| { curr_var_decl_is_visible = false; curr_var_category = INPUT_VAR; } agent_invisible_input_var
		| { curr_var_decl_is_visible = true; curr_var_category = ACTION_VAR; } avar
		| prot
		| assign 
		| init
		| trans
...
avar@init{
		...
		{ 
		  ...
		  curr_var_name = new StringArrayWA(intr, "ACT", "", null, "");
			...
			{intr.add_element(curr_module_name, $avar_type.ret, input); }
		;

avar_type returns [SMVAbstractElementInfo ret] 
		: sr= subrange 
...
prot	: { 
		...
		TOK_PROT_BEGIN prot_element_list_expr 
		{ intr.attachPROT($prot_element_list_expr.ret); }
		TOK_PROT_END  
		;

// prot_element_list_expr : prot_element_expr* // prot_else_expr ;
prot_element_list_expr returns[StmtValueArrayWA ret]
		: { Vector<CaseElement> all_elems = new Vector<CaseElement>(5);	}
		( f=prot_element_expr { all_elems.add($f.ret); } )+
		//( s=prot_else_expr { all_elems.add($s.ret); } )?
		{ $ret = intr.mk_prot(curr_module_name, all_elems, input); }
		;
		
prot_element_expr returns[CaseElement ret]
		: ^(PROT_ELEMENT_EXPR_T f=basic_expr s=basic_expr)
		{ $ret = intr.new CaseElement($f.ret, $s.ret); }
		;
		
//prot_else_expr returns[CaseElement ret]
//		: ^( PROT_ELSE_EXPR_T f=basic_expr )
//		{ $ret = intr.new CaseElement(intr.mk_constant_true_val(curr_module_name, input), $f.ret); }
//		;
...
@init {
	...
	int end_marker = -1;
			...
			: ^(VAR_DECL_T
				var_id { curr_var_name = $var_id.ret; } type)
...
@init {
	...
	int end_marker = -1;
			...
			: ^(IVAR_DECL_T var_id { curr_var_name = $var_id.ret; } type)
			{ intr.add_element(curr_module_name, $type.ret, input); }
			| ^(TOK_FOR 
				{	intr.startDummyPhase();
...
@init {
	...
	int end_marker = -1;
			...
			: ^(DEFINE_DECL_T 
				{ intr.startAllDefineScope(); }
				var_id {curr_var_name = $var_id.ret;} simple_expression
				{ intr.attachDefine($var_id.ret, $simple_expression.ret); } )
...
@init {
	...
	int end_marker = -1;
			...
			: ^(ASSIGN_DECL_T var_id simple_expression
				{ intr.attachASSIGNStmt($var_id.ret, $simple_expression.ret); } )
			| ^(INIT_ASSIGN_DECL_T var_id simple_expression
				{ intr.attachINITStmt($var_id.ret, $simple_expression.ret); } )
			| ^(NEXT_ASSIGN_DECL_T var_id next_expression
				{ intr.attachNEXTStmt($var_id.ret, $next_expression.ret); } )
			| ^(TOK_FOR 
				{ 	intr.startDummyPhase();
...
@init {
	...
	int end_marker = -1;
			...
			: ^(JUSTICE_DECL_T simple_expression
				{ intr.attachJUSTStmt($simple_expression.ret); }
				)
			| ^(TOK_FOR 
				{ 	intr.startDummyPhase();
...
@init {
	...
	int end_marker = -1;
			...
			: ^(COMPASSION_DECL_T f=simple_expression TOK_COMMA s=simple_expression
...
/* Variable identifiers.
   ...
   NEW NOTE : I'm removing the "self", and joining this type of declarations.
 */
var_id returns[StringArrayWA ret]
			: ^(REF_T var_id_helper)
