...
ctl_expr					returns[InternalSpec ret]
								: (TOK_NOT* // resolving conflict with the TOK_NOT
									...
									| TOK_EBG 
									| TOK_ABG
									| TOK_LP agent_name TOK_KNOW)) => pure_ctl_expr
...
@after { if(append_end) $ret.setEndToken($stop); if(!er()) $ret.evalBDDChildrenExp(input); }
								...
								| (TOK_LP agent_name TOK_KNOW) => ctl_know { if(!er()) $ret = $ctl_know.ret; }
...
@init {boolean append_end = false; String exp_str = ""; }
@after { if(append_end) $ret.setEndToken($stop); if(!er()) $ret.evalBDDChildrenExp(input); } 
								: TOK_LP! agent=agent_name op=TOK_KNOW^ f=ctl_root_expr TOK_RP! 
								{ 
								if (!er()) exp_str += $agent.text + " " + $op.text + " " + $f.text; 
								if(!er()) append_end = true; 
								//if(!er()) agentSpec = InitSpec.mk_ref(input, $start, $agent.text);
