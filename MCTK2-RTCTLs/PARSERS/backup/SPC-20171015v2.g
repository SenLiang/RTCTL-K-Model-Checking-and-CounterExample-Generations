...
@after { if(append_end) $ret.setEndToken($stop); if(!er()) $ret.evalBDDChildrenExp(input); }
								...
								( op=TOK_IMPLIES^ s=ctl_implies_expr //LXY: maybe a mistake
//								( op=TOK_IMPLIES^ s=ctl_iff_expr
								{ if (!er()) exp_str += " " + $op.text + " " + $s.text; if(!er()) append_end = true; if(!er()) $ret = InitSpec.mk_imply(input, $start, exp_str, $ret, $s.ret); }
								)?
								; /* right association */
...
ctl_expr					returns[InternalSpec ret]
								: (TOK_NOT* // resolving conflict with the TOK_NOT
									...
									| TOK_EBG 
									| TOK_ABG
									| TOK_LB agent_name TOK_KNOW
									)
								   ) => pure_ctl_expr { if(!er()) $ret = $pure_ctl_expr.ret; } -> ^(PURE_CTL_T pure_ctl_expr)
								| ctl_relational_expr
...
@after { if(append_end) $ret.setEndToken($stop); if(!er()) $ret.evalBDDChildrenExp(input); }
								...
								// know
								| (TOK_LB agent_name TOK_KNOW) => ctl_know { if(!er()) $ret = $ctl_know.ret; }
...
@after { if(append_end) $ret.setEndToken($stop); if(!er()) $ret.evalBDDChildrenExp(input); }
								...
								;
/*
ctl_epistemic_expr			returns[InternalSpec ret]
@init {boolean append_end = false; String exp_str = ""; }
@after { if(append_end) $ret.setEndToken($stop); if(!er()) $ret.evalBDDChildrenExp(input); }
...
@init {boolean append_end = false; String exp_str = ""; }
@after { if(append_end) $ret.setEndToken($stop); if(!er()) $ret.evalBDDChildrenExp(input); } 
								: lb=TOK_LB! agent=agent_name op=TOK_KNOW^ f=ctl_root_expr rb=TOK_RB! 
								{ 
...
@init {boolean append_end = false; String exp_str = ""; }
@after { if(append_end) $ret.setEndToken($stop); if(!er()) $ret.evalBDDChildrenExp(input); }
								: (TOK_LP TOK_ATOM TOK_KNOW) => ltl_know_expr { if (!er()) exp_str += $ltl_know_expr.text; if(!er()) $ret = $ltl_know_expr.ret; }
...
@init {boolean append_end = false; String exp_str = ""; }
@after { if(append_end) $ret.setEndToken($stop); if(!er()) $ret.evalBDDChildrenExp(input); }
								: TOK_LP agent=TOK_ATOM op=TOK_KNOW^ f=ltl_unary_expr TOK_RP
								{ if (!er()) exp_str += $agent.text + " " + $op.text + " " + $f.text; if(!er()) append_end = true; if(!er()) $ret = InitSpec.mk_ltl_know(input, $start, exp_str, $agent.text, $f.ret); }
								;
