...
import static edu.wis.jtlv.env.core.smv.schema.SMVAbstractElementInfo.NULL;
import static edu.wis.jtlv.env.core.smv.schema.SMVAbstractElementInfo.STATE_VAR;
import static edu.wis.jtlv.env.core.smv.schema.SMVAbstractElementInfo.INPUT_VAR;
import static edu.wis.jtlv.env.core.smv.schema.SMVAbstractElementInfo.ACTION_VAR;
}
...
public static int curr_var_category = NULL;
...
type returns [SMVAbstractElementInfo ret] 
		...
		{$ret = intr.mk_range_array_var(curr_var_category, curr_var_decl_is_visible, curr_var_name, $ri.ret, sr.elementAt(0), sr.elementAt(1), input); }
		;
...
mas_agent		: ^(TOK_AGENT
			agent_sign 
			{ intr.collect_module_inst(curr_module_name); agent_action_declared = false;}
			agent_declarations
			{ intr.finalize_module_inst(); }
			)
			;
...
//		| ^(AGENT_DECL_INVISIBLE_VAR_T { curr_var_decl_is_visible = false; } agent_invisible_var)	// the variables declared here are invisible
		...
		| { curr_var_decl_is_visible = true; curr_var_category = INPUT_VAR; } input_var
		| { curr_var_decl_is_visible = true; curr_var_category = ACTION_VAR; } avar_decl
		| assign 
		| init
		| trans
		| { curr_var_decl_is_visible = true; curr_var_category = NULL; } define
