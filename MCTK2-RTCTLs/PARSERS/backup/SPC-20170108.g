...
@after { if(append_end) $ret.setEndToken($stop); if(!er()) $ret.evalBDDChildrenExp(input); }
								...
								{ if (!er()) exp_str += " " + $op.text + " " + $s.text; if(!er()) append_end = true; if(!er()) $ret = InitSpec.mk_since(input, $start, exp_str, $ret, $s.ret); }
								| op=TOK_RELEASE^ s=ctls_ltl_unary_expr 
								{ if (!er()) exp_str += " " + $op.text + " " + $s.text; if(!er()) append_end = true; if(!er()) $ret = InitSpec.mk_releases(input, $start, exp_str, $ret, $s.ret); }
...
//								// epistemic operators
//								| agent=agent_name  op=TOK_KNOW^  s=ctls_ctl_expr
//								{ if (!er()) exp_str = $agent.text + " " + $op.text + " " + $s.text; if(!er()) append_end = true; if(!er()) $ret = InitSpec.mk_ctls_know(input, $start, exp_str, $agent.ret, $s.ret); }
...
//								| ctls_know primary_expr_select
//								{ if(!er()) $ret = $ctls_know.ret; } // primary_expr_select should be null...
//								-> ^(CTLS_KNOW_T ctls_know NOP primary_expr_select)
...
@after { if(append_end) $ret.setEndToken($stop); if(!er()) $ret.evalBDDChildrenExp(input); }
								...
								( op=TOK_IMPLIES^ s=atls_iff_expr
								{ if (!er()) exp_str += " " + $op.text + " " + $s.text; if(!er()) append_end = true; if(!er()) $ret = InitSpec.mk_imply(input, $start, exp_str, $ret, $s.ret); }
								)*
								; /* right association */
...
@init {boolean append_end = false; String exp_str = ""; }
@after { if(append_end) $ret.setEndToken($stop); if(!er()) $ret.evalBDDChildrenExp(input); }
								: f=atls_not_expr { if (!er()) exp_str += $f.text; if(!er()) $ret = $f.ret; }
								( op=TOK_AND^ s=atls_not_expr
								{ if (!er()) exp_str += " " + $op.text + " " + $s.text; if(!er()) append_end = true; if(!er()) $ret = InitSpec.mk_and(input, $start, exp_str, $ret, $s.ret); }
								)*
								;
								
atls_not_expr			returns[InternalSpec ret]
@init {boolean append_end = false; String exp_str = ""; }
@after { if(append_end) $ret.setEndToken($stop); if(!er()) $ret.evalBDDChildrenExp(input); }
								: atls_bounded_expr
								{ if(!er()) $ret = $atls_bounded_expr.ret; }
								| op=TOK_NOT^ f=atls_not_expr
								{ if (!er()) exp_str = $op.text + " " + $f.text; if(!er()) append_end = true; if(!er()) $ret = InitSpec.mk_not(input, $start, exp_str, $f.ret); }
								;

atls_bounded_expr				returns[InternalSpec ret]
@init {boolean append_end = false; String exp_str = ""; }
@after { if(append_end) $ret.setEndToken($stop); if(!er()) $ret.evalBDDChildrenExp(input); }
								:
								( op=TOK_OP_BFINALLY  r1=subrange f1=atls_bounded_expr
								{ if (!er()) exp_str = $op.text + " " + $r1.text + " " + $f1.text; if(!er()) append_end = true; if(!er()) $ret = InitSpec.mk_bfinally(input, $start, exp_str, $r1.ret, $f1.ret); }
								| op=TOK_OP_BGLOBALLY r2=subrange f2=atls_bounded_expr
								{ if (!er()) exp_str = $op.text + " " + $r2.text + " " + $f2.text; if(!er()) append_end = true; if(!er()) $ret = InitSpec.mk_bglobally(input, $start, exp_str, $r2.ret, $f2.ret); }
								)|(
								f=atls_binary_expr { if (!er()) exp_str += $f.text; if(!er()) $ret = $f.ret; }
								( op=TOK_BUNTIL^ r=subrange s=atls_binary_expr 
								{ if (!er()) exp_str += " " + $op.text + " " + $r.text + " " + $s.text; if(!er()) append_end = true; if(!er()) $ret = InitSpec.mk_buntil(input, $start, exp_str, $f.ret, $r.ret, $s.ret); }
								| op=TOK_BRELEASE^ r=subrange s=atls_binary_expr 
								{ if (!er()) exp_str += " " + $op.text + " " + $r.text + " " + $s.text; if(!er()) append_end = true; if(!er()) $ret = InitSpec.mk_brelease(input, $start, exp_str, $f.ret, $r.ret, $s.ret); }
								)*
								)
								;
							
atls_binary_expr				returns[InternalSpec ret]
@init {boolean append_end = false; String exp_str = ""; }
@after { if(append_end) $ret.setEndToken($stop); if(!er()) $ret.evalBDDChildrenExp(input); }
								: f=atls_unary_expr { if (!er()) exp_str += $f.text; if(!er()) $ret = $f.ret; }
								( op=TOK_UNTIL^ s=atls_unary_expr 
								{ if (!er()) exp_str += " " + $op.text + " " + $s.text; if(!er()) append_end = true; if(!er()) $ret = InitSpec.mk_until(input, $start, exp_str, $ret, $s.ret); }
								| op=TOK_SINCE^ s=atls_unary_expr 
								{ if (!er()) exp_str += " " + $op.text + " " + $s.text; if(!er()) append_end = true; if(!er()) $ret = InitSpec.mk_since(input, $start, exp_str, $ret, $s.ret); }
								| op=TOK_RELEASE^ s=atls_unary_expr 
								{ if (!er()) exp_str += " " + $op.text + " " + $s.text; if(!er()) append_end = true; if(!er()) $ret = InitSpec.mk_releases(input, $start, exp_str, $ret, $s.ret); }
								| op=TOK_TRIGGERED^ s=atls_unary_expr
								{ if (!er()) exp_str += " " + $op.text + " " + $s.text; if(!er()) append_end = true; if(!er()) $ret = InitSpec.mk_triggered(input, $start, exp_str, $ret, $s.ret); }
								// epistemic
//								| op=TOK_KNOW^ s=atls_unary_expr
//								{ if (!er()) exp_str += " " + $op.text + " " + $s.text; if(!er()) append_end = true; if(!er()) $ret = InitSpec.mk_atls_know(input, $start, exp_str, $ret, $s.ret); }
								)*
								;

/*								
atls_unary_expr				returns[InternalSpec ret]
								: (TOK_NOT* // resolving conflict with the TOK_NOT
									( TOK_OP_NEXT
									| TOK_OP_PREV
									| TOK_OP_NOTPREVNOT
									...
									| TOK_OP_HISTORICALLY
									| TOK_OP_FINALLY
									| TOK_OP_ONCE )) => atls_pure_unary_expr // all unary LTL operators 
								{ if(!er()) $ret = $atls_pure_unary_expr.ret; }
								-> ^(PURE_LTL_T atls_pure_unary_expr)
...
@after { if(append_end) $ret.setEndToken($stop); if(!er()) $ret.evalBDDChildrenExp(input); }
								: atls_state_expr
								{ if(!er()) $ret = $atls_state_expr.ret; }
								| op=TOK_OP_NEXT^ f=atls_unary_expr
								{ if (!er()) exp_str = $op.text + " " + $f.text; if(!er()) append_end = true; if(!er()) $ret = InitSpec.mk_next(input, $start, exp_str, $f.ret); }
								| op=TOK_OP_PREV^ f=atls_unary_expr
								{ if (!er()) exp_str = $op.text + " " + $f.text; if(!er()) append_end = true; if(!er()) $ret = InitSpec.mk_prev(input, $start, exp_str, $f.ret); }
								| op=TOK_OP_NOTPREVNOT^ f=atls_unary_expr
								{ if (!er()) exp_str = $op.text + " " + $f.text; if(!er()) append_end = true; if(!er()) $ret = InitSpec.mk_notprevnot(input, $start, exp_str, $f.ret); }
								| op=TOK_OP_GLOBALLY^ f=atls_unary_expr
								{ if (!er()) exp_str = $op.text + " " + $f.text; if(!er()) append_end = true; if(!er()) $ret = InitSpec.mk_globally(input, $start, exp_str, $f.ret); }
								| op=TOK_OP_HISTORICALLY^ f=atls_unary_expr
								{ if (!er()) exp_str = $op.text + " " + $f.text; if(!er()) append_end = true; if(!er()) $ret = InitSpec.mk_historically(input, $start, exp_str, $f.ret); }
								| op=TOK_OP_FINALLY^ f=atls_unary_expr
								{ if (!er()) exp_str = $op.text + " " + $f.text; if(!er()) append_end = true; if(!er()) $ret = InitSpec.mk_finally(input, $start, exp_str, $f.ret); }
								| op=TOK_OP_ONCE^ f=atls_unary_expr
								{ if (!er()) exp_str = $op.text + " " + $f.text; if(!er()) append_end = true; if(!er()) $ret = InitSpec.mk_once(input, $start, exp_str, $f.ret); }
								;

atls_state_expr			returns[InternalSpec ret]
@init {boolean append_end = false; String exp_str = ""; }
@after { if(append_end) $ret.setEndToken($stop); if(!er()) $ret.evalBDDChildrenExp(input); }
								: 
								atls_relational_expr 
								{ if (!er()) exp_str = $atls_relational_expr.text; if(!er()) $ret = $atls_relational_expr.ret; }
								| op=TOK_AA^ f=atls_state_expr 
								{ if (!er()) exp_str = $op.text + " " + $f.text; if(!er()) append_end = true; if(!er()) $ret = InitSpec.mk_allpath(input, $start, exp_str, $f.ret); }
								| op=TOK_EE^ f=atls_state_expr 
								{ if (!er()) exp_str = $op.text + " " + $f.text; if(!er()) append_end = true; if(!er()) $ret = InitSpec.mk_somepath(input, $start, exp_str, $f.ret); }
								| agent=agent_name op=TOK_KNOW^ f=atls_state_expr 
								{ if (!er()) exp_str = $agent.text + " " + $op.text + " " + $f.text; if(!er()) append_end = true; if(!er()) $ret = InitSpec.mk_atls_know(input, $start, exp_str, $agent.ret, $f.ret); }
								| agent=agent_name op=TOK_SKNOW^ f=atls_state_expr 
								{ if (!er()) exp_str = $agent.text + " " + $op.text + " " + $f.text; if(!er()) append_end = true; if(!er()) $ret = InitSpec.mk_atls_sknow(input, $start, exp_str, $agent.ret, $f.ret); }
								;
...
/*
atlsk	:	(TOK_LP atlsk TOK_RP)|
		( 
		| TOK_ATOM //primary_expr_helper1_pointer1
		| TOK_NOT atlsk
		| TOK_AA atlsk
		| TOK_EE atlsk
		| TOK_OP_NEXT atlsk
		)
		( options{greedy=true;}: (TOK_AND | TOK_OR | TOK_UNTIL) atlsk )*
		;	
*/
